...
def generate_launch_description():
    share_dir = get_package_share_directory('icp_scan_matcher')
    ...
    icp_node = launch_ros.actions.Node(
        package='icp_scan_matcher',
        executable='icp_scan_matcher',
        output="screen",
        parameters=[os.path.join(share_dir, "config", "icp_scan_matcher_param.yaml")],
        respawn=True,
    )
    return launch.LaunchDescription(
        params +
        [wit_node, base_link_to_imu_link_node,icp_node, rviz2_node]
    )