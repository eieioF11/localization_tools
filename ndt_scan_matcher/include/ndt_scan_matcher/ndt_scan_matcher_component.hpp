...
public:
  ...
  NDTScanMatcher(const std::string& name_space = "", const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
      : ExtensionNode("ndt_scan_matcher_node", name_space, options), broadcaster_(this), tf_buffer_(this->get_clock()), listener_(tf_buffer_) {
    ...
    est_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        ESTIMATE_POSE_TOPIC, rclcpp::QoS(10).best_effort(), std::bind(&NDTScanMatcher::est_pose_callback, this, std::placeholders::_1));
    est_twist_sub_ = this->create_subscription<geometry_msgs::msg::TwistStamped>(
        ESTIMATE_TWIST_TOPIC, rclcpp::QoS(10).best_effort(), std::bind(&NDTScanMatcher::est_twist_callback, this, std::placeholders::_1));
    cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        CLOUD_TOPIC, rclcpp::QoS(10).best_effort(), std::bind(&NDTScanMatcher::pointcloud_callback, this, std::placeholders::_1));
  }

  // callback

  void est_twist_callback(const geometry_msgs::msg::TwistStamped::ConstPtr msg) { estimate_twist_ = make_twist(msg->twist); }

  void est_pose_callback(const geometry_msgs::msg::PoseStamped::ConstPtr msg) { estimate_pose_ = make_pose(msg->pose); }
