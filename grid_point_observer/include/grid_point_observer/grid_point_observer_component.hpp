...
public:
  ...
  GridPointObserverNode(const std::string& name_space = "", const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
      : ExtensionNode("grid_point_observer_node", name_space, options), broadcaster_(this), tf_buffer_(this->get_clock()), listener_(tf_buffer_) {
    ...
    odom_sub_       = this->create_subscription<nav_msgs::msg::Odometry>(ODOM_TOPIC, rclcpp::QoS(10),
                                                                   std::bind(&GridPointObserverNode::odometry_callback, this, std::placeholders::_1));
    laser_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        LASER_POSE_TOPIC, rclcpp::QoS(10).best_effort(), std::bind(&GridPointObserverNode::laser_pose_callback, this, std::placeholders::_1));
    imu_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        IMU_TOPIC, rclcpp::QoS(10).best_effort(), std::bind(&GridPointObserverNode::imu_callback, this, std::placeholders::_1));
    // timer
